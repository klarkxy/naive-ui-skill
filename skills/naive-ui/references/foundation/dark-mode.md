...
- [Required Reading Router](#required-reading-router)
...
- [Common Dark Theme Variables](#common-dark-theme-variables)
...
> 本文件是 dispatcher；权威细节在原"API Reference / Common Patterns / Best Practices"段中，按下面任务对照读取。

| 任务 | 必须读 |
| --- | --- |
| 跟随 OS 主题 | 本 skill 的 "useOsTheme" 段 |
| 自定义主色在暗色下不生效 | 本 skill 的 "Custom Primary Color in Dark Mode" 段 |
| 主题切换持久化 | 本 skill 的 "Persistent Theme" 段 |
| 完整 dark theme overrides | 本 skill 的 "Custom Theme Overrides" 段 |
...
- 自定义主色在暗色下"不糊"必须同时设置 `primaryColorSuppl`。仅设置 `primaryColor` 不会自动推导 `primaryColorHover` / `primaryColorPressed`，dark theme 下会出现 hover/pressed 颜色错误。
- 最小覆盖示例：
...
const overrides = {
  common: {
    ...
    primaryColorSuppl: '#4098fc'  // 暗色下必须
  }
}
</script>
...
> 完整推理与变体：见 `naive-ui/references/core-theme.md`（"Theme Colors Not Applied in Dark Mode" 段）与 `naive-ui/references/core-troubleshooting.md`。
